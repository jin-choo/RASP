...
if  a==0%  || ExpAB<=5  || ExpAB>=(min(sum(couple))-5)     %case of no coincidences or limit for the F asinptotical distribution (too few coincidences)
    ...
    if reference_lag<=0 
        if l_==0
            ...
            TPrMTot=[0 ctAB(1); ExpAB 0]; % matrix with #AB and #BA
        elseif l_>0      
            for i=1:maxrate  
                sA=Zaa{i}(:,1);
                sB=Zaa{i}(:,2);         
            ...
            TPrMTot=[0 ctAB(J); ExpAB 0]; % matrix with #AB and #BA 
        else
            for i=1:maxrate  
                sA=Zaa{i}(:,1);
                sB=Zaa{i}(:,2);        
        ...
        TPrMTot=[0 Hab; ExpAB 0]; % matrix with #AB and #BA 
    end
