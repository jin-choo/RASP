function [assembly]=Main_assemblies_detection(spM,MaxLags,BinSizes,ref_lag,alph,No_th,O_th,bytelimit,filepathprint,RecordingTime)
...
%         fname = sprintf('C:/Users/Hyunjin/Desktop/20-brain-image/brainimage/cad/%d_%d_%.2f_%d.mat', RecordingTime, int * 1000, alph, O_th);
%         parsave(fname,assemblybin{gg})
%         if O_th == 5
        fileID = fopen(filepathprint,'a');
        fprintf(fileID, sprintf("%d,%d,%.2f,%d,%d\n", RecordingTime, int * 1000, alph, O_th, RunningTime)); 
        fclose(fileID);
%         end
    end
end
