import numpy as np, matplotlib.pyplot as plt, yaml #, neo, quantities as pq, elephant.conversion as conv
...
def cad_read(k_tuples: int, file_path: str, count_threshold: int, probabilistic_participation: int):
    # np.random.seed(1)
    ...
    if file_path[:2] == 'gt':
        ...
        params["recording"]["recording_time"] = 500
        ...
        # bst = BinnedSpikeTrain(spiketrains, bin_size=bin_size * pq.ms)
        ...
        # patterns = cell_assembly_detection(bst, max_lag=6, reference_lag=6, alpha=1, min_occurrences=count_threshold, size_chunks=params["recording"]["recording_time"] * 1000 / bin_size, subgroup_pruning=False)
        ...
        for k_tuple in tqdm(range(1, k_tuples)):
            patterns = loadmat(f"./generated_data/{file_path}_{probabilistic_participation}_{k_tuple + 1}")['aus'][0][0][0][0]
            ...
            for pattern in patterns:
                ...
                itemset_array = pattern[0][0][0][0]
                if len(itemset_array) == k_tuple + 1:
                    support = pattern[0][0][-1][0][-1]
                    pvalue = pattern[0][0][2][0][0]
                    itemset_number = itemset_np_to_number(max_neuron, k_tuple + 1, itemset_array)
            ...
            fpr = [1]
            tpr = [1]
            f = open(f"./txt/cad/{file_path}_{probabilistic_participation}_{k_tuple + 1}_support_acc.txt", 'w')
            ...
            plt.savefig(f"./figure/cad/{file_path}/{file_path}_{probabilistic_participation}_{k_tuple + 1}_support.png", bbox_inches='tight')
            plt.close()

            fpr = [1]
            tpr = [1]
            f = open(f"./txt/cad/{file_path}_{probabilistic_participation}_{k_tuple + 1}_pvalue_acc.txt", 'w')
            f.write(f"threshold, recall, precision, f1_score\n")
            ...
            plt.title(f"{k_tuple + 1}-Tuple Count ROC Curve", fontsize=23)
            plt.legend(fontsize=15)
            plt.savefig(f"./figure/cad/{file_path}/{file_path}_{probabilistic_participation}_{k_tuple + 1}_pvalue.png", bbox_inches='tight')
            plt.close()
        ...
        #         fpr = []
        #         tpr = []
        #         f = open(f"./txt/spade/{file_path}_{probabilistic_participation}_{k_tuple + 1}_pvalue_acc.txt", 'w')
        #         f.write(f"threshold, recall, precision, f1_score\n")
        #
        #         enumerate_pvalue_list = enumerate(pvalue_list)
        #         for threshold in np.unique(pvalue_list)[::-1]:
        ...
        #         plt.title(f"{k_tuple + 1}-Tuple Count ROC Curve", fontsize=23)
        #         plt.legend(fontsize=15)
        #         plt.savefig(f"./figure/spade/{file_path}_{probabilistic_participation}_{k_tuple + 1}_pvalue.png")
        #         plt.close()