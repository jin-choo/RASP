import math, numpy as np, yaml, mat73, scipy.io as sio, os, csv, datetime
...
def neurons_number_to_unsorted_neurons(number, max_neuron, k_tuple):
    ...
    for k_neurons in range(1, k_tuple + 1):
        ...
        neurons.append(round(q))
    return neurons


def cad_data(motif_type, NIDs, recording_time, background_firing_rate, motif_firing_rate, probabilistic_participation, temporal_jitter, time_warping, motif_neurons_numbers, motif_max_lags, motif_max_spikes, seed):
    ...
    length = max(map(len, spike_time))
    spM = np.array([xi + [np.nan] * (length - len(xi)) for xi in spike_time])
    os.makedirs(f"./generated_data", exist_ok=True)
    sio.savemat(f"./generated_data/{motif_type}_NIDs_{NIDs}_time_{recording_time}_bg_{background_firing_rate:.2f}_pp_{probabilistic_participation:.1f}_tj_{temporal_jitter}_tw_{time_warping}_mt_{motif_firing_rate:.2f}_{motif_neurons_numbers[0]}_{motif_neurons_numbers[1]}_{motif_max_lags}_{motif_max_spikes}_seed_{seed}.mat", {"gt": spM})


def cad_read_ndcg_rc(motif_type, NIDs, recording_time, background_firing_rate, motif_firing_rate, probabilistic_participation, temporal_jitter, time_warping, motif_neurons_numbers, motif_max_lags, motif_max_spikes, bin_size, alpha, option, seed, top_K_list, i_exp):
    max_lag = max(math.ceil(100.0 / bin_size), 3)
    ...
    if os.path.exists(f"./cad_stat{option}/{motif_type}/{motif_type}_NIDs_{NIDs}_time_{recording_time}_bg_{background_firing_rate:.2f}_pp_{probabilistic_participation:.1f}_tj_{temporal_jitter}_tw_{time_warping}_mt_{motif_firing_rate:.2f}_{motif_neurons_numbers[0]}_{motif_neurons_numbers[1]}_{motif_max_lags}_{motif_max_spikes}_bin_{bin_size}_lag_{max_lag}_alpha_{alpha:.2f}_{k + 1}_seed_{seed}.mat"):
        os.makedirs(f"./txt/cad_stat{option}/{motif_type}", exist_ok=True)
        os.makedirs(f"./figure/cad_stat{option}/{motif_type}", exist_ok=True)
        ...
        patterns = mat73.loadmat(f"./cad_stat{option}/{motif_type}/{motif_type}_NIDs_{NIDs}_time_{recording_time}_bg_{background_firing_rate:.2f}_pp_{probabilistic_participation:.1f}_tj_{temporal_jitter}_tw_{time_warping}_mt_{motif_firing_rate:.2f}_{motif_neurons_numbers[0]}_{motif_neurons_numbers[1]}_{motif_max_lags}_{motif_max_spikes}_bin_{bin_size}_lag_{max_lag}_alpha_{alpha:.2f}_{k + 1}_seed_{seed}.mat")['aus']

        for pattern in patterns:
            itemset_array = pattern['elements']
            if len(itemset_array) == k + 1:
                pvalue = pattern['pr']
                itemset_number = itemset_to_number(NIDs, k + 1, sorted([item - 1 for item in itemset_array]))
                try:
        ...
        for top_K in top_K_list:
            ...
            f_ndcg = open(f"./txt/cad_stat{option}/{motif_type}_cad_stat{option}_ndcg_{i_exp}.csv", 'a')
            f_ndcg.write(f'{motif_type},{NIDs},{recording_time},{background_firing_rate:.2f},{probabilistic_participation:.1f},{temporal_jitter},{time_warping},{motif_firing_rate:.2f},{motif_neurons_numbers[0]},{motif_neurons_numbers[1]},{motif_max_lags},{motif_max_spikes},{bin_size},{max_lag},{alpha:.2f},{k + 1},{top_K},{ndcg},{seed}\n')
            f_ndcg.close()

            f_rc = open(f"./txt/cad_stat{option}/{motif_type}_cad_stat{option}_rc_{i_exp}.csv", 'a')
            f_rc.write(f'{motif_type},{NIDs},{recording_time},{background_firing_rate:.2f},{probabilistic_participation:.1f},{temporal_jitter},{time_warping},{motif_firing_rate:.2f},{motif_neurons_numbers[0]},{motif_neurons_numbers[1]},{motif_max_lags},{motif_max_spikes},{bin_size},{max_lag},{alpha:.2f},{k + 1},{top_K},{rc},{seed}\n')
            f_rc.close()
...
def cad_data_case():
    ...
    with open('./txt/datasets/ecommerce/yoochoose-clicks_crop.dat', 'r') as input_file:
        ...
        for row in reader:
            time = datetime.datetime.strptime(row[1], "%Y-%m-%dT%H:%M:%S.%fZ")
            if time < time_min or time >= time_max:
                continue
            item = int(row[2])
            if item in item_list:
                item_times[item_dict[item]].add((time - time_min).total_seconds())

    for item, item_time in enumerate(item_times):
        item_times[item] = sorted(item_time)

    length = max(map(len, item_times))
    spM = np.array([xi + [np.nan] * (length - len(xi)) for xi in item_times])
    os.makedirs(f"./generated_data", exist_ok=True)
    sio.savemat(f"./generated_data/yoochoose-clicks_crop_time_{time_span}_item_{len(item_list)}.mat", {"gt": spM})


def cad_read_case(binSize, alpha, option):
    time_min = datetime.datetime(2014, 4, 27, 0, 0, 0, 0)
    time_max = datetime.datetime(2014, 4, 28, 0, 0, 0, 0)
    ...
    if os.path.exists(f'./cad_stat{option}/yoochoose-clicks_crop_time_86400_item_{item_index}_length_{max_pattern}_bin_{binSize * 1000}_lag_{winlen}_alpha_{alpha:.2f}.mat'):
        session_dict = dict()
        ...
        patterns = mat73.loadmat(f'./cad_stat{option}/yoochoose-clicks_crop_time_86400_item_{item_index}_length_{max_pattern}_bin_{binSize * 1000}_lag_{winlen}_alpha_{alpha:.2f}.mat')['aus']
        for pattern in patterns:
            itemset_array = pattern['elements']
            if len(itemset_array) == k + 1:
                support = pattern['Noccurrences']
                neurons_number = itemset_to_number(NIDs, k + 1, [item - 1 for item in itemset_array])
                try:
        ...
        print()
        print(time_span, binSize, winlen, option, alpha)
