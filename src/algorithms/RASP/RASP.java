...
public class RASP {
	 ...
	 */
	public void runAlgorithm(int motifType, Integer NIDs, Integer recordingTime, double backgroundFiringRate, double pp, Integer temporalJitter, Integer timeWarping, double motifFiringRate, int motifNeurons, int motifNumbers, int motifMaxLags, int motifMaxSpikes, Integer binSize, Integer winLen, Integer rangeBins, long timestamp, int seed) throws IOException {  //, float minObsExp
		...
		Files.createDirectories(Paths.get(String.format("./TSPs/%d_RASP_1", motifType)));
		String output = String.format("./TSPs/%d_RASP_1/%d_RASP_NIDs_%d_time_%d_bg_%.2f_pp_%.1f_tj_%d_tw_%d_mt_%.2f_%d_%d_%d_%d_bin_%d_winlen_%d_interlen_%d_inter_%d_seed_%d", motifType, motifType, NIDs, recordingTime, backgroundFiringRate, pp, temporalJitter, timeWarping, motifFiringRate, motifNeurons, motifNumbers, motifMaxLags, motifMaxSpikes, binSize, winLen, interLen, rangeBins, seed);  // the path for saving the frequent itemsets found
		...
		BufferedWriter writerTime = new BufferedWriter(new FileWriter(String.format("./txt/%d_RASP_time_%s.csv", motifType, new SimpleDateFormat( "MM.dd_HH.mm").format(timestamp)), true));
	...
	void generateCandidate2() throws IOException {
		...
		for (FPNode item1Node : root.childs) {
			...
			for (int item2Item = 0; item2Item < maxItem; item2Item++) {
				...
				short item2 = (short) (item2Item * winLen);
			...
			for (int item2Bin = rangeBins; item2Bin <= interLen; item2Bin = item2Bin + rangeBins) {
				for (int item2Item = 0; item2Item < maxItem; item2Item++) {
					if (item1Item == item2Item) {
						continue;
					}
					...
					BitSet item2TIDs = mapItemTIDs.get(item2);
					BitSet item2TIDsRange = mapItemTIDs.get((short) (item2 - rangeBins));
			...
			if (item2Node != null) {
				for (FPNode item3Node : item2Node.childs) {
					...
					if (item3Bin >= rangeBins) {
						itemTIDsRange = mapItemTIDs.get((short) (item - rangeBins));
					}
