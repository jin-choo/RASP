BinSizes = [1.0 2.0 3.0 5.0 10.0 15.0 20.0 30.0 60.0];
...
starttime = now;
fileID = fopen(sprintf('/home/dmlab/hyunjin/brainimage/cad_stat5/yoochoose-clicks_crop_time_%s.csv', datestr(starttime, 'mm.dd_HH.MM')),'w');
...
parfor iTable=1:height(Table)
    ...
    MotifTypeprint = sprintf("/home/dmlab/hyunjin/brainimage/cad_stat5/yoochoose-clicks_crop_time_%s.csv", datestr(starttime, 'mm.dd_HH.MM'));
    ...
    try
        ...
        for jj=1:length(BinSizes)
            ...
            fname = sprintf("/data/hyunjin/brainimage/cad_stat5/yoochoose-clicks_crop_time_%d_item_%d_length_%d_bin_%d_lag_%d_alpha_%.2f.mat", Time, Item, Length, int, MaxLags(jj), alph);
